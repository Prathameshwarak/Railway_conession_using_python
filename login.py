...
class Login:
    def __init__(self, root):
        ...
        self.usertext.place(x=400, y=60, width=200, height=25)
        ...
        self.passwordText.place(x=400, y=100, width=200, height=25)
        ...
        self.role_menu.place(x=400, y=140, width=200, height=25)
    ...
    def login(self):
        ...
        if not re.match(email_pattern, email):
            messagebox.showerror("Error", "Invalid email format. Please enter a valid email (e.g., user@example.com).")
            return

        # Password Validation
        if len(password) < 10 or not re.search(r"[A-Z]", password) or not re.search(r"[a-z]", password) or not re.search(r"[@#$%^&+=!]", password):
            messagebox.showerror("Error", "Password must be at least 10 characters long and contain an uppercase letter, a lowercase letter, and a special character.")
            return
